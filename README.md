...
- 선정 이유
  - 사용자 입장에서 정보를 입력하고 수정하는 것보다 바로 결과를 보여줌으로 사용자 편의성과 정확한 데이터를 수집하기 위해서 개선해야 할 부분이라고 생각합니다.
- 개선 방향
  - 유효성 검사가 모두 통과된 경우 다음 버튼이 활성화되며, 버튼을 클릭하면 프로필 설정 화면이 나타나도록 적용해보고 싶습니다.
- 추후 관련 지식 습득을 위한 키워드
  - html, javascript
...
- 선정 이유
  - 비밀번호를 암호화를 고려하지 않고 DB에 저장하여 데이터 유출의 위험과 사용자의 신뢰도가 떨어지게 되는 아쉬움이 남았습니다.
- 개선 방향
  - SHA-256, bcrypt, PBKDF2, Argon2 등과 같은 안전한 해시 함수를 사용하여 비밀번호를 해시화해 저장합니다.
  - 솔트, 페퍼를 추가하고 사용하여 더욱 안전한 사용자 인증 시스템을 구축할 수 있습니다.
- 추후 관련 지식 습득을 위한 키워드
  - 해시 함수 사용, 솔트, 페퍼
